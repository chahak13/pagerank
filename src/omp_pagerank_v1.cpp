...
#pragma omp parallel for reduction(+ : sum_pr, dangling_pr) schedule(runtime)
...
#pragma omp parallel for schedule(runtime)
    for (size_t i = 0; i < num_rows; ++i) {
      ...
      for (size_t k = 0; k < rows[i].size(); k++) {
        double h_i =
            (num_outgoing[rows[i][k]]) ? 1.0 / num_outgoing[rows[i][k]] : 0.0;
...
#pragma omp parallel for reduction(+ : error) schedule(runtime)
    for (size_t i = 0; i < num_rows; ++i) {
      error = error + fabs(pr[i] - old_pr[i]);
    }
